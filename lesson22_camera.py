...
class Lesson22Camera(BaseCamera):
    ...
    def initialize_windows(self):
        ...
        #Position the 4 smaller frames
        quarter_height = int(self.HEIGHT/2)
        total_height = self.top_bar #move the window down so that it aligns with the main window
        for region in self.regions:
            #print(self.rois[region])
            cv2.namedWindow(region, cv2.WINDOW_GUI_NORMAL)
            cv2.resizeWindow(region, int(self.WIDTH/2), int(self.HEIGHT/2))
            #position the windows to the right of the main window
            cv2.moveWindow(region,int(self.WIDTH), total_height)
            total_height = total_height + quarter_height + self.window_waste
    ...
    def show_windows(self):
        ...
        w2 = int(self.WIDTH // 2)
        quadrants = [
            self.frame[0:h2, 0:w2],
            self.frame[0:h2,h2:self.WIDTH],
            self.frame[h2 :self.HEIGHT, 0:w2],
            self.frame[h2:self.HEIGHT, w2:self.WIDTH]
        ]
        
        for region, frame in zip(self.regions, quadrants):
